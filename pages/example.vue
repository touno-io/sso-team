...
export default Vue.extend({
  ...
  async asyncData ({ $cookiz, $api, env }) {
    ...
    const auth = { ssoUrl: env.ssoUrl, loggedIn: !!token, user: null, token, error: '' }
    if (auth.loggedIn) {
      try {
        // const { data } = await $api.get(`${env.baseUrl}/v1/auth/account`, {
        //   headers: { Authorization: `Bearer ${token}` }
        // })
        const { data } = await $api.request(`GET /v1/auth/account`, { headers: { Authorization: `Bearer ${token}` } })
        auth.user = data
  ...
  methods: {
    ...
    onSignIn () {
      ...
      sso.signInRedirect(this.auth.ssoUrl, this.appId, this.appUrl, this.getOnceKey())
    },
    onSignOutCancel () {
...
<template>
  <div class="row">
    <div class="col-sm-22 col-36 mb-3 mb-sm-1">
      ...
      <div>Single Sign-On (SSO) solutions provide users with an easier way to access all of their accounts. Implementing these solutions allows you to access all of your accounts, with just one set of credentials.</div>
    </div>
    <div v-if="!auth.loggedIn" class="col-sm-14 col-36">
      <h5>Guest User</h5>
      <div>Please click Sign-In Button.</div>
      <button type="button" class="btn btn-primary mt-3" :disabled="sign.redirected" @click="onSignIn">
        <fa :icon="`fa-solid ${sign.redirected?'fa-sync':'fa-arrow-right-to-bracket'}`" class="mr-2" :class="{'fa-spin':sign.redirected}" />
        <span v-text="sign.redirected?'Redirect...':'Sign-In TOUNO.io'" />
      </button>
    </div>
    <div v-else class="col-sm-14 col-36">
      <img :src="`${auth.user.gravatar}?s=220&d=404`" class="rounded-circle mb-3" style="width: 120px" :alt="auth.user.name" />
      <h5 v-text="`Welcome, ${auth.user.name}`" />
      <p class="text-muted">{{auth.user.job}} <span class="badge bg-info text-white" v-text="auth.user.level" /></p>
      <button type="button" class="btn mt-3" :class="!sign.approved?'btn-warning':'btn-danger'" :disabled="sign.reload" @click="onSignOutConfirm">
        <fa :icon="`fa-solid ${!sign.approved?'fa-arrow-right-from-bracket':'fa-circle-check'}`" class="mr-2" />
        <span v-text="sign.approved ? sign.reload ? 'Good bye...' : 'Confirm' : 'Logout'"></span>
      </button>
      <button v-show="sign.approved" type="button" class="btn btn-secondary ml-1 mt-3" @click="onSignOutCancel">
        <span>Cancel</span>
      </button>
    </div>
  </div>
